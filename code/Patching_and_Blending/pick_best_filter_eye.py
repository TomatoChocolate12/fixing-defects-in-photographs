...
class BlendshapeExtractor:
    """
    Class for extracting blendshape values (e.g., eye blink) from facial landmarks.
    ...
    def calculate_blendshapes(self, image_path):
        ...
        if blendshapes:
            eyeBlendshapes['eyeBlinkLeft'] = blendshapes['eyeBlinkLeft']
            eyeBlendshapes['eyeBlinkRight'] = blendshapes['eyeBlinkRight']

        return eyeBlendshapes
...
class ImageProcessor:
    """
    Class for processing images, calculating poses, and filtering the images based on pose similarity and eye blink threshold.

    Args:
        base_image_path (str): Path to the base image.
        cluster_folder_path (str): Path to the folder containing cluster images.
        threshold (float): Threshold for the sum of eyeBlinkLeft and eyeBlinkRight blendshape values.
    ...
    Methods:
        ...
        sort_and_filter(pose_data): Sorts pose data by similarity to the base pose and filters based on the eye blink threshold.
        write_pose_data(pose_data): Writes pose and blendshape data to the CSV file.
        run(): Runs the entire process of creating the CSV file, processing images, sorting, and filtering.
    """
    def __init__(self, base_image_path, cluster_folder_path, threshold):
        self.base_image_path = base_image_path
        self.cluster_folder_path = cluster_folder_path
        self.threshold = threshold
        self.csv_file_path = os.path.join(cluster_folder_path, f"{os.path.basename(cluster_folder_path)}_eyes_csv.csv")
    ...
    def create_csv_file(self):
        with open(self.csv_file_path, 'w', newline='') as csvfile:
            ...
            writer.writerow(['path/to/file', 'x', 'y', 'z', 'eyeBlinkLeft', 'eyeBlinkRight'])
            ...
            if filename.endswith('.jpg') or filename.endswith('.png') or filename.endswith('.JPG') or filename.endswith('.jpeg'):
                ...
                if (x != 0 and y != 0 and z != 0):
                    ...
                    with open(self.csv_file_path, 'a', newline='') as csvfile:
                       writer = csv.writer(csvfile)
                       writer.writerow([image_path, x, y, z, blendshapes['eyeBlinkLeft'], blendshapes['eyeBlinkRight']])
    ...
    def read_pose_data(self):
       ...
       with open(self.csv_file_path, 'r') as csvfile:
           ...
           for row in reader:
               image_path, x, y, z, eye_blink_left, eye_blink_right = row
               x, y, z = float(x), float(y), float(z)
               eye_blink_left, eye_blink_right = float(eye_blink_left), float(eye_blink_right)
               pose_data.append((image_path, x, y, z, eye_blink_left, eye_blink_right))
       return pose_data
    ...
    def sort_and_filter(self, pose_data):
       ...
       pose_data = [data for data in pose_data if (data[4] + data[5]) / 2 <= self.threshold]
       return pose_data
    ...
    def run(self):
       ...
       filtered_pose_data = self.sort_and_filter(pose_data)
       self.write_pose_data(filtered_pose_data)
...
def main():
   ...
   parser.add_argument('--threshold', type=float, default=0.1, help='Threshold for eyeBlinkLeft and eyeBlinkRight sum')
   args = parser.parse_args()

   image_processor = ImageProcessor(args.base_image_path, args.cluster_folder_path, args.threshold)
   image_processor.run()
