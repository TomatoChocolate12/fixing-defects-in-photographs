        ...
        for idx in range(len(paths_list[0])):
            ...
            if self.opt.save_test_gif:
                ...
                for face_idx in range(len(faces_list) - 1):  # remove target image
                    cur_numpy = np.array(self.visual.numpy2im(faces_list[face_idx][idx], resize=True))
                ...
                concate_img = np.array(self.visual.numpy2im(faces_list[0][idx], resize=True))
                for face_idx in range(1, len(faces_list)):
                    concate_img = np.concatenate(
                        (concate_img, np.array(self.visual.numpy2im(faces_list[face_idx][idx], resize=True))), axis=1)
